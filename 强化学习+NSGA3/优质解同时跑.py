...
# 示例数据生成（假设X有三个特征，y为二元标签）

X_dcs = data.iloc[:, :-1]  # 所有行，除最后一列外的所有列作为特征
y_dcs = data.iloc[:, -1]  # 最后一列作为标签
# 数据分割
...
# --------------------------------------------------------------------------------
class DQN(nn.Module):
    def __init__(self, state_dim=3, action_dim=11):          # 两个状态，11个动作
...
def get_state(population):
    N = len(population)
    temp = -population                     # 取反，获取正常的目标值

    temp = (temp - np.min(np.array(y), axis=0)) / (np.max(np.array(y), axis=0) - np.min(np.array(y), axis=0))  # 要取负号，因为代理模型输出的是相反数，y的负号，然后取最值，要注意
    # 归一化后 预测
    ...
    if N == 0:
        return (0.0, 0.0, 0.0)
    population = (population - np.min(-np.array(y), axis=0)) / (np.max(-np.array(y), axis=0) - np.min(-np.array(y), axis=0))   # 要取负号，因为代理模型输出的是相反数，y的负号，然后取最值，要注意
...
def train_dqn(model_name):
    ...
    epsilon_start = 1.0
    epsilon_end = 0.2
    epsilon_decay = 1000  # 调整衰减速度

    target_update_interval = 10  # 每隔多少个 episode 同步一次目标网络
    ...
    for episode in range(num_episodes):
        ...
        if (episode+1)%5==0:
            torch.save(policy_net.state_dict(), 'dqn_policy-Hqs-another5-reload-'+str(episode+1)+'.pth')
...
if __name__ == "__main__":
    ...
    # model_name = 'dqn_policy-Hqs-test5.pth'
    ...
    for i in range(20,201,5):
        ...
        np.random.seed(1)
        moedel_x = i
        model_name = 'dqn_policy-Hqs-another5-reload-'+str(moedel_x)+'.pth'
