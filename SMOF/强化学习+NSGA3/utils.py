...
def funfun(N):
    ...
    low=np.zeros((1,D))
    up=np.ones((1,D))
    pop = np.tile(low,(N,1))+(np.tile(up,(N,1))-np.tile(low,(N,1)))*np.random.rand(N,D)            # 生成参考点大小的种群
    return pop
...
def cal(pop,func):
    ...
    popfun = np.vstack((array1, array2, array3)).T
    return popfun
...
def GO(pop,t1,t2,pc,pm):
    ...
    #模拟二进制交叉
    beta=np.zeros((N,D))
    mu=np.random.random_sample([N,D])                       # 均匀分布的0-1之间的数据
...
def envselect(mixpop,N,Z,Zmin,M,D,func):
    #非支配排序
    mixpopfun = cal(mixpop,func)
