...
def train_dqn(model_name):
    ...
    for episode in range(num_episodes):
        ...
        if (episode+1)%5==0:
            torch.save(policy_net.state_dict(), 'dqn_policy-Hqs-test-0.5gamm5.26-'+str(episode+1)+'.pth')
...
if __name__ == "__main__":
    ...
    model_name = 'dqn_policy-Hqs-test-gam-0.5-5.26-.pth'
    rewards = train_dqn(model_name)
    save_HV = []
    save_IGD = []
    for i in range(5,201,5):
        # moedel_x = 85
        model_name = 'dqn_policy-Hqs-test-0.5gamm5.26-'+str(i)+'.pth'
    ...
    for i in range(len(save_HV)):
        print('第{name}组：'.format(name=i*5))
        print('超体积：', save_HV[i])
