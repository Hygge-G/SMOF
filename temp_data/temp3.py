...
三个模型，单个目标，只优化权重        针对目标3，线性回归，KNN，决策树，
'''
count = 0
r2_3 = []
rmse_3 = []
...
'''
最优解 x: [0.11194707 0.10875673 0.80637722]
最优值 f(x): [0.00645849]
'''
temp_np = np.array([r2_3])
temp_np.pop()
avera_r2 = temp_np.reshape(100,50).mean(axis=1)
print(avera_r2.shape)  # 输出 (300,)
temp_np = np.array([rmse_3])
...
# 保存图像（需在plt.show()之前）
plt.tight_layout()
plt.savefig('y3图片四合一.png', dpi=300, bbox_inches='tight')
# 显示图像
plt.show()